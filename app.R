...
ui <- fluidPage(
  ...
  div(style = "padding: 1px 0px; width: '100%'",
      titlePanel(
        title = "",
        windowTitle = "Species genetic diversity analysis"
      )
  ),
  navbarPage(
    title = div(
      span(
        HTML("<input type=button style='font-size:30px;border:0;height:35px' value='Species genetic diversity analysis' onclick=\"window.history.go(-1)\">"),
    ...
    tabPanel(
      ...
      fluidRow(
        column(
          ...
          actionButton(
            'run',
            'Run (species genetic diversity analysis)!'
          )
        )
      )
    ),
...
server <- function(input, output, session) {
  ...
  observeEvent(input$run, {
    ...
    froot = currentFile()
    withProgress(message="Eigentree:", value=0, {
      n=4
      # Chr me 
      #froot = currentFile()
      ...
      for(i in 1:autosome){
        # Chromosome number
        Chr_Mark_Num_cmd1 = paste0("cat ",froot,".3.core.bim | grep -w '^",i,"' | wc -l")
        Chr_Mark_Num1 = system(Chr_Mark_Num_cmd1,intern = TRUE)
        Chr_Mark_Num1 <- as.numeric(Chr_Mark_Num1)
        # Determine whether chromosomes exist
        if(Chr_Mark_Num1==0){
          next
        }
        # Chromosome level GRM
        for(j in 1:autosome){
          Chr_Mark_Num_cmd_2 = paste0("cat ",froot,".3.core.bim | grep -w '^",j,"' | wc -l")
          Chr_Mark_Num2 = system(Chr_Mark_Num_cmd_2,intern = TRUE)
          Chr_Mark_Num2 <- as.numeric(Chr_Mark_Num2)
          if(Chr_Mark_Num2==0){
            next
          }
          if(i<=j){
            if(i==j){
              # Chromosome number
              Chr_Mark_Matr[i,i] <- Chr_Mark_Num1
              # GRM construct
              ...
              Chr_Mark_Num = Chr_Mark_Num1+Chr_Mark_Num2
              Chr_Mark_Matr[i,j] <- Chr_Mark_Num
              # GRM construct
              incProgress(2/n, detail = paste0(" making grm for chromosome ",i," and ",j," ..."))
              GRM_cmd = paste0(plink2, " --bfile ",froot,".3.core --chr ",i,",",j," --chr-set 90 --allow-extra-chr --allow-no-sex --make-grm-gz --out ",froot,".chr",i,"_",j)
              system(GRM_cmd)
              gz=gzfile(paste0(froot,".chr",i,"_",j, ".grm.gz"))
              grm=read.table(gz, as.is = T)
              offDiag = grm[grm[,1]!=grm[,2], 4]
              Me=1/var(offDiag/sc, na.rm = TRUE)
        ...
        Chr_Me_Matr_Final <- adjustdata(Chr_Me_Matr)
        colnames(Chr_Me_Matr_Final)[1] <- ""
        Chr_Mark_Matr_Final <- adjustdata(Chr_Mark_Matr)
        colnames(Chr_Mark_Matr_Final)[1] <- ""
        # Output chr level Me and Marker
        Output1 <- paste0(froot,".Chr_Marker.txt")
        Output2 <- paste0(froot,".Chr_Me.txt")
        write.table(Chr_Mark_Matr_Final,file=Output1,quote = FALSE,sep="\t",row.names = FALSE)
        write.table(Chr_Me_Matr_Final,file=Output2,quote = FALSE,sep="\t",row.names = FALSE)
        # LD calculate
        ...
        for(i in 1:nrow(LD)){
          ...
          # within chr  1/me
          if(i==j){
            LD[i,j] <- 1/Chr_Me_Matr[i,i]
          }else{
            # between chr (m^2/me-m1^2/me1-m2^2/me2)/2m1*m2 
        ...
        colnames(LD_Final)[1] <- ""
        Output3=paste0(froot,".Chr_LD.txt")
      ...
      withProgress(message="Visualization:", value=0, {
        ...
        output$figure <- downloadHandler(
          filename = function(){
            paste0('Decomp_of_me_fig.zip')
          },
          content = function(file) {
              fs <- c(paste0(froot,".Chr_LD.png"),paste0(froot,".Chr_LD_Scale.png"))
              zip(zipfile = file, files = fs)
            }
        )
        output$LD <- downloadHandler(
          filename = function(){
           paste0("Decomp_of_me_txt.zip")
          },
          content = function(file) {
              fs <- c(paste0(froot,".Chr_LD.txt"),paste0(froot,".Chr_LD_Scale.txt"))
              zip(zipfile = file, files = fs)
            }
        )
